...
``coma`` goes one step further. With it, commands determine which configuration
...
Why choose ``coma`` over another ``omegaconf``-based solution, like
`hydra <https://hydra.cc>`_? ``hydra``, specifically, has the following
...
``hydra`` (or any other framework). ``hydra``, in particular, certainly has a
